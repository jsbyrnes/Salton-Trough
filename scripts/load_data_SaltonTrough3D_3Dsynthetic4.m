function [ dataStruct ] = load_data_SaltonTrough3D_3Dsynthetic4( TD_parameters )
...
%             tS    = [ tS (tS + pert_ssip) ];
    ...
    Qmodel = (1/150)*ones(length(xVec), length(yVec), length(zVec));
    
    Qmodel(xVec<0, :, zVec< 80)                        = 1/2000;
    Qmodel(xVec>70, :, zVec<120)                       = 1/1000;
    Qmodel(xVec>=0 & xVec<=70, :, zVec>=40 & zVec<=160) = 1/75;
