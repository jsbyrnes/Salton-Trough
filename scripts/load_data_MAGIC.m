...
%     for k = 1:3
%     
%     	allTS(dataE == k) = allTS(dataE==k) + normrnd(0,0.02, size(allTS(dataE==k)));
%     
%     end
%     
%     for k = 4:6
...
%     allLats = linspace(min(allLats), max(allLats), length(allLats));
    allTS(:) = normrnd(0, 0.05, size(allTS)) + (( 0.4/(max(allLons) - min(allLons)) *(allLons - min(allLons))) - 0.2);
