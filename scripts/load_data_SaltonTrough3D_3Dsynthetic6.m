function [ dataStruct ] = load_data_SaltonTrough3D_3Dsynthetic6( TD_parameters )
...
%         end
    ...
    Qmodel = (5.5/1000)*ones(length(xVec), length(yVec), length(zVec));
        
    Qmodel(xVec>=0   & xVec<=30, :, zVec>=60 & zVec<=90) = 3.5/1000;
    Qmodel(xVec>=-50 & xVec<=-20, :, zVec>=60 & zVec<=90) = 7.5/1000;

    [Y, X, Z] = meshgrid(yVec, xVec, zVec);
    
    F    = scatteredInterpolant(X(:), Y(:), Z(:), Qmodel(:), 'linear');
    ...
    for k = 1:length(dataStruct.dtS)
        ...
        dataStruct.dtS(k) = sum(rayL.*rayQinv.*U);
